...
-- | Example usage and tests for capability negotiation

-- | Example: Negotiate between a full-featured client and server
exampleFullNegotiation :: Capabilities
exampleFullNegotiation =
  let clientCaps =
        buildClientCapabilities
          ClientCapabilityBuilder
            { ccbExperimental = Nothing,
              ccbSampling = Just SamplingCapability
            }
      serverCaps = fullServerCapabilities
   in negotiateCapabilities clientCaps serverCaps

-- | Example: Negotiate when client has limited capabilities
exampleLimitedClientNegotiation :: Capabilities
exampleLimitedClientNegotiation =
  let clientCaps =
        Capabilities
          (Just $ KeyMap.fromList [(fromText "tools", toJSON $ ToolsCapability Nothing)])
          Nothing
      serverCaps = fullServerCapabilities
   in negotiateCapabilities clientCaps serverCaps

-- | Example: Negotiate with minimal server
exampleMinimalServerNegotiation :: Capabilities
exampleMinimalServerNegotiation =
  let clientCaps = defaultClientCapabilities
      serverCaps = minimalServerCapabilities
   in negotiateCapabilities clientCaps serverCaps

-- | Test case: Client with no declared capabilities should get basic features
testBasicCapabilities :: Bool
testBasicCapabilities =
  let clientCaps = defaultClientCapabilities -- No explicit capabilities
      serverCaps = fullServerCapabilities
      result = negotiateCapabilities clientCaps serverCaps
   in hasToolsSupport result && hasResourcesSupport result && hasPromptsSupport result

-- | Test case: Client with only tools should get only tools
testToolsOnlyNegotiation :: Bool
testToolsOnlyNegotiation =
  let clientCaps =
        Capabilities
          (Just $ KeyMap.fromList [(fromText "tools", toJSON $ ToolsCapability Nothing)])
          Nothing
      serverCaps = fullServerCapabilities
      result = negotiateCapabilities clientCaps serverCaps
   in hasToolsSupport result && not (hasResourcesSupport result) && not (hasPromptsSupport result)

-- | Test case: Empty server capabilities should result in empty negotiation
testEmptyServerNegotiation :: Bool
testEmptyServerNegotiation =
  let clientCaps = defaultClientCapabilities
      serverCaps = minimalServerCapabilities
      result = negotiateCapabilities clientCaps serverCaps
   in not (hasToolsSupport result)
        && not (hasResourcesSupport result)
        && not (hasPromptsSupport result)
        && not (hasLoggingSupport result)
