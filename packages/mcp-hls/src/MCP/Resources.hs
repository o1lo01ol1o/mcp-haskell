...
encodeProjectInfo :: Utils.FileSystem.ProjectInfo -> Text
encodeProjectInfo Utils.FileSystem.ProjectInfo {..} =
  let json =
        object
...
getWorkspaceSymbols _projectRoot = do
  result <- return $ Left "Workspace symbols not implemented" -- HLS.Client.getWorkspaceSymbols "" -- Empty query gets all symbols
  case result of
    Left err -> do
