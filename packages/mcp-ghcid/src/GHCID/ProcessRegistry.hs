...
instance Show GHCIDHandle where
  show GHCIDHandle{..} = "GHCIDHandle{cabalURI=" ++ show ghcidCabalURI ++ 
                         ", workDir=" ++ show ghcidWorkDir ++ 
                         ", startTime=" ++ show ghcidStartTime ++ "}"

instance Eq GHCIDHandle where
  h1 == h2 = ghcidCabalURI h1 == ghcidCabalURI h2 && 
...
shutdownProcessRegistry registry@ProcessRegistry{..} = do
  ...
  when (not $ null failures) $
    logWarn $ "Failed to stop " <> T.pack (show $ length failures) <> " processes during shutdown"
...
createGHCIDHandle cabalURI workDir = do
  ...
  let processConfig = proc "ghcid" ["--command", "cabal repl"]
