...
defaultServerConfig = GHCIDServerConfig
  ...
  , instructionsMessage = T.unlines
      ...
      , "Note: default `cabal repl` invocations isolate build artifacts under $TMPDIR/mcp-cache/cabal-build/ (or MCP_CACHE_DIR); include an explicit `--builddir` in `options.command` to override."
