...
import Control.Monad (forM, replicateM, replicateM_, void, when)
...
spec = describe "GHCID.Concurrency" $ do
  describe "ProcessRegistry concurrent access" $ do
    it "handles concurrent process starts safely" $
      withTestTimeout 45 $
    ...
    it "handles concurrent start/stop operations safely" $
      withTestTimeout 40 $
    ...
    it "maintains consistency under concurrent list operations" $
      withTestTimeout 35 $
    ...
    it "handles process health checks concurrently" $
      withTestTimeout 30 $
  ...
  describe "STM-based operations under high concurrency" $ do
    ...
    it "handles resource contention gracefully" $
      withTestTimeout 30 $
  ...
  describe "Stress testing under load" $ do
    ...
    it "maintains performance under concurrent health monitoring" $
      withTestTimeout 40 $
