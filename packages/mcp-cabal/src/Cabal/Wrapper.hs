...
parentCheckIntervalMicros = 500000
...
parseWrapperArgs args =
  case break (== "--") args of
    ...
    _ -> Left "Expected `--parent-pid <pid> -- <command>`"
  where
...
startCabalProcess :: String -> [String] -> IO (Process () () ())
startCabalProcess cmd cmdArgs = do
  let processConfig =
        proc cmd cmdArgs
          & setStdin inherit
          & setStdout inherit
...
runCabalWrapper :: [String] -> IO ()
runCabalWrapper args =
  case parseWrapperArgs args of
    Left err -> do
      hPutStrLn stderr $ "mcp-cabal wrapper error: " <> err
      exitFailure
    Right (parentPid, cmd, cmdArgs) -> do
      ...
      outcome <-
        mask $ \restore ->
          bracket (startCabalProcess cmd cmdArgs) (cleanupProcess groupId) $ \process -> do
            ...
            case result of
              ...
              Right (Right ()) -> do
                hPutStrLn stderr "mcp-cabal wrapper: parent process exited unexpectedly; terminating cabal"
                pure ParentLost
      case outcome of
